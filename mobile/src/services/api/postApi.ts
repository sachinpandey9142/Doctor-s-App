...
export const getFeedPosts = async (page = 1, limit = 10): Promise<FeedResponse> => {
...
export const getCasePosts = async (page = 1, limit = 10): Promise<FeedResponse> => {
...
export const likePostRequest = async (postId: string) => {
  const response = await apiClient.post<ApiResponse<{ postId: string; liked: boolean; likesCount: number }>>(
    `/posts/${postId}/like`
  );
  return response.data.data;
};

export const commentPostRequest = async (postId: string, text: string): Promise<Comment> => {
  const response = await apiClient.post<ApiResponse<Comment>>(`/posts/${postId}/comment`, { text });
  return response.data.data;
};

export const getPostCommentsRequest = async (postId: string): Promise<Comment[]> => {
