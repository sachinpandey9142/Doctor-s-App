...
import { ArrowLeft, SendHorizontal, Stethoscope, Users } from "lucide-react-native";
...
export function CaseDiscussionThreadScreen() {
  ...
  const renderMessage = useCallback(
    ({ item, index }: { item: Message; index: number }) => {
      ...
      return (
        <Animated.View
          entering={(isMine ? FadeInRight : FadeInLeft).duration(200).springify().damping(18)}
          style={[styles.msgRow, isMine ? styles.msgRowRight : styles.msgRowLeft]}
        >
          {!isMine && (
            <Avatar name={item.senderId.name} uri={item.senderId.profileImage} size={32} />
          )}
          <View style={[styles.msgBubble, isMine ? styles.msgBubbleMine : styles.msgBubbleOther]}>
            ...
            {isMine ? (
              <LinearGradient colors={["#2563EB", "#06B6D4"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.bubbleGradient}>
                <Text style={styles.msgTextMine}>{item.text}</Text>
                <Text style={styles.msgTimeMine}>{formatRelativeTime(item.createdAt)}</Text>
              </LinearGradient>
            ) : (
              <View style={styles.bubbleOtherInner}>
                <Text style={[styles.msgText, { color: theme.colors.textPrimary }]}>{item.text}</Text>
                <Text style={[styles.msgTime, { color: theme.colors.textTertiary }]}>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      {/* ── Header ─────────────────────────────────────────────────── */}
      <View style={[styles.header, { paddingTop: insets.top + 10, backgroundColor: theme.colors.surface, borderBottomColor: theme.colors.borderLight }]}>
        ...
        <View style={styles.headerTitle}>
          <View style={styles.headerIconRow}>
            ...
            <Text style={[styles.headerLabel, { color: "#7C3AED" }]}>CASE DISCUSSION</Text>
          </View>
      ...
      {/* ── Case Preview Card ────────────────────────────────────── */}
      {caseSnippet ? (
        <View style={[styles.casePreview, { backgroundColor: "#FAF5FF", borderColor: "#E9D5FF" }]}>
          <Stethoscope size={12} color="#7C3AED" />
          <Text style={styles.casePreviewText} numberOfLines={2}>{caseSnippet}</Text>
        </View>
      ) : null}
      ...
      >
          ...
          ListEmptyComponent={
            <View style={styles.emptyWrap}>
              ...
              <Text style={styles.emptyBody}>
                Share your thoughts, questions, or insights on this clinical case.
              </Text>
            </View>
          }
        />
        ...
        {/* ── Input ────────────────────────────────────────────────── */}
        <View style={[styles.inputBar, { backgroundColor: theme.colors.surface, borderTopColor: theme.colors.borderLight, paddingBottom: insets.bottom + 8 }]}>
          <TextInput
            style={[styles.input, { color: theme.colors.textPrimary, backgroundColor: theme.colors.background, borderColor: theme.colors.border }]}
          ...
          >
            <LinearGradient colors={["#2563EB", "#06B6D4"]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.sendGradient}>
...
const styles = StyleSheet.create({
  ...
  backBtn: { padding: 4 },
  headerTitle: { flex: 1 },
  headerIconRow: { flexDirection: "row", alignItems: "center", gap: 5, marginBottom: 2 },
  headerIcon: {
    width: 20, height: 20, borderRadius: 6,
    backgroundColor: "#EDE9FE",
    alignItems: "center", justifyContent: "center"
  },
  headerLabel: { fontFamily: "Manrope_700Bold", fontSize: 9.5, letterSpacing: 1 },
  ...
  msgTextMine: { fontFamily: "Manrope_500Medium", fontSize: 15, lineHeight: 22, color: "#FFFFFF" },
  msgTime: { fontFamily: "Manrope_500Medium", fontSize: 10, marginTop: 3, textAlign: "left" },
  msgTimeMine: { fontFamily: "Manrope_500Medium", fontSize: 10, marginTop: 3, color: "rgba(255,255,255,0.6)", textAlign: "right" },
  // Empty
  emptyWrap: { alignItems: "center", paddingTop: 60, paddingHorizontal: 40 },
  emptyTitle: { fontFamily: "SpaceGrotesk_700Bold", fontSize: 18, color: "#0F172A", marginTop: 14 },
  emptyBody: { fontFamily: "Manrope_500Medium", fontSize: 14, color: "#64748B", textAlign: "center", marginTop: 8, lineHeight: 22 },
  // Input
  inputBar: {
    flexDirection: "row",
    alignItems: "flex-end",
