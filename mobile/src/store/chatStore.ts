...
const upsertMessage = (messages: Message[], message: Message) => {
  const next = messages.filter((item) => item._id !== message._id);
  next.push(message);
  ...
  openOrCreateConversation: async (participantId) => {
    ...
    set((state) => ({
      conversations: sortConversations(
        state.conversations.some((item) => item._id === conversation._id)
  ...
  appendIncomingMessage: ({ conversationId, message }) => {
    set((state) => {
      ...
      return {
        messagesByConversation: {
          ...state.messagesByConversation,
          [conversationId]: upsertMessage(currentMessages, message),
        },
        conversations: sortConversations(
          state.conversations.map((conversation) =>
            conversation._id === conversationId
